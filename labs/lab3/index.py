...
def gen_trajectory(POMDP, x0, n):
    ...
    for i in range(0, n):
        ...
        # a = np.random.choice(len(A), p=A_p)
        ...
        # print(f'np.diag(O[a][:,z].shape): {np.diag(O[a][:, z]).shape}')
        # print(f'np.diag(O[a][:,z]): \n {np.diag(O[a][:, z])}')
        ...
        norm = np.linalg.norm(belief, axis=0, keepdims=True, ord=1)
        # print(norm)
        belief = belief / norm
        # print(belief)

        print(belief.shape)
        x = np.random.choice(len(X), p=belief)
        # x = np.argmax(belief)
